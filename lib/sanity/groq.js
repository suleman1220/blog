...
*[_type == "post" && featured] | order(publishedAt desc, _createdAt desc) {
  _id,
  _createdAt,
  publishedAt,
  mainImage {
...
export const paginatedquerywithsearch = groq`
*[_type == "post" && _score > 0]
| score(title match $query || excerpt match $query)
| order(_score desc, publishedAt desc, _createdAt desc) [$pageIndex...$limit] {
  ...,
  _score,
  author->,
  categories[]->
